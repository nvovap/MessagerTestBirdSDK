...
//        self.present(vc, animated: true, completion: nil)
    ...
    func didStartReconnection() {
        if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
            DispatchQueue.main.async {
                (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format:"Group Channel (%ld)", self.groupChannel.memberCount), subTitle: "Reconnecting...")
            }
        }
    }
            ...
            if error == nil {
                DispatchQueue.main.async {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
                        if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
                            (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format:"Group Channel (%ld)", self.groupChannel.memberCount), subTitle: "")
                        }
                    }
                }
            }
    ...
    func didFailReconnection() {
        if self.navItem.titleView != nil && self.navItem.titleView is UILabel {
            DispatchQueue.main.async {
                (self.navItem.titleView as! UILabel).attributedText = Utils.generateNavigationTitle(mainTitle: String(format: "Group Channel (%ld)", self.groupChannel.memberCount), subTitle: "Reconnection Failed. Tap to reconnect.")
            }
        }
    }
